...
function parseData (s : string) : number {
  ...
  const winningCards : number[] = s1[0]; const ownedCards : number[] = s1[1]
  return Math.floor(2 ** (ownedCards.filter(s => winningCards.includes(s)).length - 1))
}
...
function solution1(inputStr : string) : number {
  const initArray = inputStr.split('\n').filter(x => x.length !== 0).map(x => parseData1(x))
