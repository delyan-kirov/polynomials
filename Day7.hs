...
joke (Hand a1 a2 a3 a4 a5) = do
  let handList = [a1, a2, a3, a4, a5]
      ...
      h = (if not . null $ nonJs then (do
              let maxNonJ = fst $
                            maximumBy (\(_, x) (_, y) -> compare x y) $
                            Map.assocs $
                            Map.delete J multiSet
              (\x -> if x == J then maxNonJ else x) <$> handList) else handList)
  Hand (head h) (h !! 1) (h !! 2) (h !! 3) (h !! 4)
...
solution_1 s = do
  let hand_bet_s = sortBy (\(x, _) (y, _) -> compare x y) $ parseInput s
